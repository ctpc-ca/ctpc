...
def run_swiss_tournament(bots, num_rounds, top_k, verbose=False, *, initial, scoring,
    ...
    """
    Parallel Swiss: identical pairing/scoring behavior to your original,
    but matches within each round are executed in sequential batches
    (each batch runs in parallel up to the configured worker limit).
    """
    ...
    try:
        ...
        # Batch size - defaults to max_workers, can be smaller/larger via env
        try:
            matches_per_batch = int(os.getenv("CTPC_MATCHES_PER_SET", str(max_workers)))
        except ValueError:
            matches_per_batch = max_workers
        matches_per_batch = max(1, matches_per_batch)

        for round_num in range(num_rounds):
            print(f"--- Running Round {round_num + 1} of {num_rounds} ---")

            # Build score groups once per round
            ...
            for group_score in sorted(score_groups.keys(), reverse=True):
                ...
                while i < len(group):
                    ...
                    # 1) Try pairing bots with same score first
            ...
            # Parallel execution, collect (starter, responder, outcome, moves, pgn_str)
            results_this_round = []
            ...
            if scheduled_matches:
                print(f"SCHEDULED MATCHES THIS ROUND: {len(scheduled_matches)}")
                print(f"BATCH SIZE: {matches_per_batch}")

                for batch_idx, batch in enumerate(_chunked(scheduled_matches, matches_per_batch), 1):
                    pool_size = min(max_workers, len(batch))
                    print(f"  -> Running batch {batch_idx} with {pool_size} workers ({len(batch)} matches)")
                    with ThreadPoolExecutor(max_workers=pool_size) as pool:
                        futures = [pool.submit(_worker, sm) for sm in batch]
                        for fut in as_completed(futures):
                            results_this_round.append(fut.result())

            # Fold results back after compelting matches
        ...
        # SB tiebreaks from pairings log
        get_sb(scores, pairings_log)
...
def get_sb(scores, pairings_log):
    # Recompute SB from the final scores and the actual results log
    for b in scores:
        scores[b]["sb"] = 0.0

    final_pts = {b: scores[b]["score"] for b in scores}

    for round_pairs in pairings_log:
        for starter, responder, result in round_pairs:
            if result == '1':
                scores[starter]["sb"] += final_pts[responder]
            elif result == '2':
                scores[responder]["sb"] += final_pts[starter]
            else:
                scores[starter]["sb"]  += 0.5 * final_pts[responder]
                scores[responder]["sb"] += 0.5 * final_pts[starter]
    return scores
